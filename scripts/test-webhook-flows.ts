...
const createSamplePayload = (walletAddress: string) => {
  return [
    {
...
2. Run with ts-node: npx ts-node scripts/test-webhook-flow.ts
3. Optionally set environment variables:
   - SERVER_URL: URL of your server
   - WALLET_ADDRESS: Address to use in the test

Example:
SERVER_URL=http://167.88.169.171:3000 npx ts-node scripts/test-webhook-flow.ts
*/ 
